...
type AllAnalysisProps = {
  className?: string;
  onAddEmpleado: (nombre: string, index: number) => void;
  indiceActivo: number;
  empleadosYaAsignados: string[];
};

export function AllAnalysis({
  className,
  onAddEmpleado,
  indiceActivo,
  empleadosYaAsignados,
}: AllAnalysisProps) {
  ...
  const handleAddEmployee = (employeeName: string) => {
    if (!addedEmployees.includes(employeeName)) {
      ...
      onAddEmpleado(employeeName, indiceActivo);
      closeModal();
    }
  };
  ...
  const sortedData = [...data]
    .filter((emp) => !empleadosYaAsignados.includes(emp.employee_name))
    .sort((a, b) => {
      if (!sortKey) return 0;
      ...
      <Table>
        <TableHeader>
          <TableRow className="border-none uppercase text-center dark:hover:bg-transparent">
            ...
            <TableHead className="px-2 py-1 text-center select-none">Acción</TableHead>
          </TableRow>
        </TableHeader>
