...
interface ProyectoModalProps {
  projectId: string;
  onClose: () => void;
  onUnapply: (projectId: string) => void;
      ...
      >
        ...
        {filter === "pending" && (
          <div className="flex justify-end">
            {deleteProjects.includes(project.projectId) ? (
              <button
                onClick={() => onUnapply(project.projectId)}
                className="bg-[#65417f] hover:bg-[#5a366e] text-white font-medium px-6 py-2 rounded-lg transition"
              >
                Cancelar desaplicación
              </button>
            ) : (
              <button
                onClick={() => onUnapply(project.projectId)}
